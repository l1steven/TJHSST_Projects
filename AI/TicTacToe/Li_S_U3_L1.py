...
def minimax(state, turn, tc):
   ...
   # v=max_value
   # successors=successors(state,turn)
   # st=successors[0][1]
...
def max_value(state, turn, tc):
   ...
   for a,s in successors(state,turn):
      # m=min_value(s,switch_turn(turn),tc)[0]
      # if v<m:
      #    v=m
      #    st=s
      #v=max(v,min_value(s,switch_turn(turn),tc)[0])
      li.append((min_value(s,switch_turn(turn),tc)[0],s))
   return max(li,key=itemgetter(0))
   
def min_value(state, turn, tc):
   # return value and state: (val, state)
   if terminal_test(state,tc): return utility(switch_turn(turn),tc,state),state
   #v=1
   #st=state
   li=[]
   for a,s in successors(state,turn):
      # m=max_value(s,switch_turn(turn),tc)[0]
      # if v>m:
      #    v=m
      #    st=s
      #v=min(v,max_value(s,switch_turn(turn),tc)[0])
      li.append((max_value(s,switch_turn(turn),tc)[0],s))
   return min(li,key=itemgetter(0))
