...
class HeapPriorityQueue():
   ...
   def heapUp(self, k):
      if k==1: return
      while self.queue[k][0] < self.queue[k//2][0]: 
          self.swap(k,k//2)
          k=k//2
          if k==1: return
   ...
   def heapDown(self, k, size):
        ...
        if left>size: return 
        min=right 
        if left== size : min=left
        elif self.queue[right][0]>self.queue[left][0]: min=left
        if self.queue[min][0]<self.queue[k][0]: 
            self.swap(k,min)
   ...
   def remove(self, index):
      # Your code goes here
      pop=self.queue[index]       # change this
      self.queue[index]=self.queue.pop()
...
def make_graph(nodes = "rrNodes.txt", node_city = "rrNodeCity.txt", edges = "rrEdges.txt"):
   ...
   # Your code goes here
   rrnode=open(nodes,"r")
   rrnodecity=open(node_city,"r")
   rredges=open(edges,"r")
   for line in rrnode.readlines():
      l=line.split()
      coord=(float(l[1]),float(l[2]))
      nodeLoc[l[0]]=coord
   for line in rrnodecity.readlines():
      l=line.split()
      s=l[1]
      ...
      nodeToCity[l[0]]=s
      cityToNode[s]=l[0]
   for line in rredges.readlines():
      l=line.split()
      if l[0] not in neighbors:
         neighbors[l[0]] = [l[1]] 
      else:
         neighbors[l[0]].append(l[1])
      if l[1] not in neighbors:
         neighbors[l[1]]=[l[0]]
      else:
         neighbors[l[1]].append(l[0])
      c=calc_edge_cost(nodeLoc[l[0]][0],nodeLoc[l[0]][1],nodeLoc[l[1]][0],nodeLoc[l[1]][1])
      edgeCost[(l[0],l[1])]=c
      edgeCost[(l[1],l[0])]=c
...
def dist_heuristic(n1, n2, graph):
   ...
   coord1=loc[n1]
   coord2=loc[n2]
   return calc_edge_cost(coord1[0],coord1[1],coord2[0],coord2[1])
   # Your code goes here
...
def display_path(path, graph):
   li=[]
   for node in path:
      if node in graph[1]: li.append(graph[1][node])
   # Your code goes here
   print("Length of the path:" + str(len(path)))
   print(li)
...
def generate_path(state, explored, graph):
   ...
   n=state
   cost = 0
   while n != "s": #"s" is initial's parent
      path.append(n)
      tup=(n,explored[n])
      if(explored[n]!="s"):
         cost=cost+graph[4][tup]
      n = explored[n]
...
def findPath(start,goal,met,explored,backexplored,graph):
   fhalf=[]
   bhalf=[]
   n=met
   cost=0
   ...
   while n!="s":
      bhalf.append(n)
      if backexplored[n]!="s":
         tup=(n,backexplored[n])
         cost=cost+graph[4][tup]
      n=backexplored[n]
   fhalf=fhalf[::-1]
   fhalf.pop()
   return fhalf+bhalf , cost
...
def draw_final_path(ROOT, canvas, path, graph, col='red'):
   ...
   for i in range(len(path)-1):
      node1=path[i]
      node2=path[i+1]
      coord1=graph[5][node1]
      coord2=graph[5][node2]
      drawLine(canvas,*coord1,*coord2,col)
   ROOT.update()
...
def bi_bfs(start, goal, graph, col):
   ...
   while len(frontier)!=0 and len(frontier)!=0: 
      ...
      if forwardpath in backtier: 
         path,cost= findPath(start,goal,forwardpath,explored,backexplored,graph)
         draw_final_path(ROOT, canvas, path, graph)
         return path, cost
      for child in graph[3][forwardpath]:
         if child not in explored: 
            frontier.append(child)
            explored[child]=forwardpath
            drawLine(canvas, *graph[5][forwardpath], *graph[5][child], col)
      backpath=backtier.pop(0)
      if backpath in frontier: 
         path,cost= findPath(start,goal,backpath,explored,backexplored,graph)
         draw_final_path(ROOT, canvas, path, graph)
         return path, cost
      for child in graph[3][backpath]:
         if child not in backexplored:
            backtier.append(child)
            backexplored[child]=backpath
            drawLine(canvas, *graph[5][backpath], *graph[5][child], col)
      counter += 1
      if counter % 1000 == 0: ROOT.update()
   return None
...
def a_star(start, goal, graph, col, heuristic=dist_heuristic):
   ...
   while not frontier.isEmpty(): 
       ...
       #print(path[1])
       if path[1]==goal:
         draw_final_path(ROOT, canvas, path[2], graph)
         return path[2],path[3]
       for child in graph[3][path[1]]:
            g=path[3]+graph[4][(path[1],child)]
            h=heuristic(child,goal,graph)        
            if child not in explored or explored[child]>h+g: 
               frontier.push((h+g,child,path[2]+[child],g))
               explored[child]=h+g
               drawLine(canvas, *graph[5][path[1]], *graph[5][child], col)
       counter += 1
       if counter % 1000 == 0: ROOT.update()
   return None
...
def bi_a_star(start, goal, graph, col, ROOT,canvas,heuristic=dist_heuristic):
   ...
   frontier[1].push((cost,goal))
   explored=[{start:(0,[start])}, {goal:(0,[goal])}]
   f_set, sg=[{start},{goal}],[start,goal]
   counter=0
   x=1
   while not frontier[0].isEmpty() and not frontier[1].isEmpty():
      ...
      f_set[x]-= {s}
      if s in f_set[1-x]:
         path, cost=explored[x][s][1][:-1]+explored[1-x][s][1][::-1], explored[x][s][0]+explored[1-x][s][0]
         draw_final_path(ROOT,canvas,path,graph)
         return path,cost
      for child in graph[3][s]:
         g=explored[x][s][0]+graph[4][(s,child)]
         h=heuristic(child,sg[1-x],graph)
         if child not in explored[x] or explored[x][child][0]>g:
            frontier[x].push((g+h,child))
            f_set[x].add(child)
            explored[x][child]=(g,explored[x][s][1]+[child])
...
def tri_directional(city1, city2, city3, graph, col,ROOT,canvas,heuristic=dist_heuristic):
   ...
   cost,path=min([cost12+cost23 , path12[:-1]+path23], [cost23+cost31 , path23[:-1]+path31], [cost31+cost12 , path31[:-1]+path12])
   draw_final_path(ROOT,canvas,path,graph)
   return path,cost
...
def main():
   ...
   draw_all_edges(ROOT, canvas, graph)
   cur_time = time.time()
   path, cost = bi_a_star(graph[2][start], graph[2][goal], graph, 'orange',ROOT,canvas) #root, canvas
   if path != None: display_path(path, graph)
   else: print ("No Path Found.")
